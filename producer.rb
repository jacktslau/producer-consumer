...
class Producer
  ...
  def run
    ...
    if(!updated_acc.nil?)
      ...
      @logger.info "Producer ##{pid} generates $#{transaction.amount} #{transaction.type} transaction ##{transaction.id} to Account ##{updated_acc.id} with updated balance: #{updated_acc.balance}"
    end
  end
